...
async function getGames(category) {
    ...
    // بناء الرابط بناءً على التاب المختار
...
// لتحميل تفاصيل اللعبة عند النقر عليها
...
// عند الضغط على أي تاب يتم تغيير الـ API حسب الفئة المحددة
document.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', (e) => {
        const category = e.target.getAttribute('data-category')
        getGames(category)  // استدعاء الـ API مع تغيير الفئة
    })
})

// تحميل الألعاب عند تحميل الصفحة مع الفئة الافتراضية
getGames('mmorpg')
